...
class Query(graphene.ObjectType):
    ...
    def resolve_tags_by_color(self, info, color=None):
        ...
        cached_result = cache.get(cache_key)
        if cached_result:
            logger.info(f"🔄 Tags par couleur depuis le cache: {len(cached_result)} tags")
            return cached_result
    ...
    def mutate(self, info, name, color):
        ...
        try:
            ...
            try:
                ...
                logger.info(f"🔄 Cache invalidé via le gestionnaire pour Tag ID: {tag.pk}")
            except Exception as cache_error:
                logger.warning(f"⚠️  Erreur d'invalidation du cache manager: {cache_error}")
...
def demonstrate_cache_issue():
    ...
    if result5.data['createTagWithCacheInvalidation']['success']:
        ...
        print(f"   ✅ Tag créé avec invalidation: {new_tag2['name']} (ID: {new_tag2['id']})")
    ...
    print(f"Tags après création sans invalidation: {len(tags_after_without_invalidation)}")
    ...
    if len(tags_after_without_invalidation) == len(tags_before):
        ...
        print("   → Les mutations ne déclenchent pas l'invalidation automatique du cache")
