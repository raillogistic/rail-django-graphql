...
jobs:
  ...
  quality:
    ...
    - name: Analyse de complexité
      run: |
        radon cc django_graphql_auto --json > complexity.json
        radon mi django_graphql_auto --json > maintainability.json
...
class CITestRunner:
    ...
    def run_all_tests(self):
        ...
        test_suites = {
            'linting': 'flake8 django_graphql_auto tests',
            'formatting': 'black --check django_graphql_auto tests',
            'imports': 'isort --check-only django_graphql_auto tests',
            'typing': 'mypy django_graphql_auto',
            'security': 'bandit -r django_graphql_auto',
