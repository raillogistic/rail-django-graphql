...
```python
# django_graphql_auto/generators/introspector.py
...
```python
# django_graphql_auto/generators/types.py
import graphene
...
```python
# django_graphql_auto/generators/queries.py
import graphene
...
from django_graphql_auto.extensions.base import BaseExtension
...
from django_graphql_auto.extensions.caching import BaseCacheExtension
...
class TestPerformance(TestCase):
    ...
    @override_settings(DJANGO_GRAPHQL_AUTO={'ENABLE_QUERY_OPTIMIZATION': True})
...
```python
# django_graphql_auto/extensions/optimization.py
...
```python
# django_graphql_auto/extensions/caching.py
from django.core.cache import cache
...
# Run linting
flake8 django_graphql_auto/
black django_graphql_auto/
...
3. **Run Quality Checks**
   ...
   # Check code style
   black --check django_graphql_auto/
   flake8 django_graphql_auto/
   mypy django_graphql_auto/
   
   # Check security
   bandit -r django_graphql_auto/
   ```
...
def analyze_query_performance(query: str):
    ...
    from django_graphql_auto.extensions.optimization import QueryAnalyzer
