...
class Brand(models.Model):
    ...
    country = models.ForeignKey(
        Country, on_delete=models.CASCADE, verbose_name="Pays d'origine"
    )
...
class CustomerProfile(models.Model):
    ...
    country = models.ForeignKey(
        Country, on_delete=models.CASCADE, verbose_name="Pays de résidence"
    )
...
class Order(models.Model):
    ...
    customer = models.ForeignKey(
        Customer, on_delete=models.CASCADE, related_name="orders", verbose_name="Client"
    )
...
def create_nested_filters():
    ...
    basic_generator = AdvancedFilterGenerator(
        max_nested_depth=2, enable_nested_filters=True
    )
    ...
    sample_filters = [f for f in product_filter_set.base_filters.keys() if "__" in f][
        :10
    ]
    ...
    deep_generator = AdvancedFilterGenerator(
        max_nested_depth=3, enable_nested_filters=True
    )
    ...
    deep_filters = [
        f for f in order_filter_set.base_filters.keys() if f.count("__") >= 2
    ][:5]
    ...
    conservative_generator = AdvancedFilterGenerator(
        max_nested_depth=1, enable_nested_filters=True
    )

    customer_filter_set = conservative_generator.generate_filter_set(Customer)
    print(
        f"Customer filters (depth 1): {len(customer_filter_set.base_filters)} filters"
    )
...
class Query(graphene.ObjectType):
    ...
    def resolve_products(self, info, filters=None):
        ...
        queryset = Product.objects.select_related(
            "category", "category__parent", "brand", "brand__country"
        )
    ...
    def resolve_orders(self, info, filters=None):
        ...
        queryset = Order.objects.select_related(
            "customer", "customer__profile", "customer__profile__country"
        ).prefetch_related(
            "items",
            "items__product",
    ...
    def resolve_customers(self, info, filters=None):
        ...
        """
        queryset = Customer.objects.select_related("profile", "profile__country")
...
def performance_optimization_examples():
    ...
    def get_optimized_products_queryset():
        ...
        return Product.objects.select_related(
            ...
            "orderitem_set", "orderitem_set__order", "orderitem_set__order__customer"
        )
    ...
    class PerformanceOptimizedFilterGenerator(AdvancedFilterGenerator):
        ...
        def generate_filter_set(self, model, current_depth=0):
            ...
            print(
                f"Generated {filter_count} filters for {model.__name__} "
        ...
        def _suggest_query_optimizations(self, model, filters):
            ...
            if select_related_fields:
                suggestions.append(
                    f"select_related({', '.join(repr(f) for f in select_related_fields)})"
                )

            if prefetch_related_fields:
                suggestions.append(
                    f"prefetch_related({', '.join(repr(f) for f in prefetch_related_fields)})"
                )
