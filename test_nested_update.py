...
def test_nested_update_functionality():
    ...
    comment1 = Comment.objects.create(content="First comment", author=user, post=post)

    comment2 = Comment.objects.create(content="Second comment", author=user, post=post)
    ...
    if result3.errors:
        ...
        update_post_mutation = next(
            (m for m in mutations if m["name"] == "updatePost"), None
        )

        if update_post_mutation:
            input_arg = next(
                (arg for arg in update_post_mutation["args"] if arg["name"] == "input"),
                None,
            )
            if input_arg and input_arg["type"]["inputFields"]:
                ...
                nested_comments = next(
                    (
                        f
                        for f in input_arg["type"]["inputFields"]
                        if f["name"] == "nested_comments"
                    ),
                    None,
                )
                if nested_comments:
