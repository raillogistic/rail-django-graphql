...
def debug_post_save(sender, instance, created, **kwargs):
    ...
    print(f"🔥 POST_SAVE signal fired: {sender.__name__} - {instance} (created: {created})")
...
def debug_m2m_changed(sender, instance, action, pk_set, **kwargs):
    ...
    print(f"🔥 M2M_CHANGED signal fired: {sender.__name__} - {instance} (action: {action})")
...
def check_middleware_signals():
    ...
    try:
        ...
        for receiver in post_save._live_receivers(sender=Category):
            if 'GraphQLCacheInvalidationMiddleware' in str(receiver) or 'cache_invalidation' in str(receiver):
...
def run_debug_test():
    ...
    print(f"🔥 post_save déclenché: {'✅ OUI' if signals_fired['post_save'] else '❌ NON'}")
    print(f"🔥 post_delete déclenché: {'✅ OUI' if signals_fired['post_delete'] else '❌ NON'}")
    print(f"🔥 m2m_changed déclenché: {'✅ OUI' if signals_fired['m2m_changed'] else '❌ NON'}")
    
    if details:
        print("\n📋 DÉTAILS DES SIGNAUX DÉCLENCHÉS:")
        for detail in details:
            print(f"   {detail['time']} - {detail['signal']} - {detail['sender']} - {detail.get('action', 'N/A')}")
    
    print("\n" + "=" * 80)
    
    if middleware_connected and signals_fired['post_save'] and signals_fired['post_delete']:
