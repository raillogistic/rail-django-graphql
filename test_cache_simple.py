...
def test_cache_behavior():
    ...
    response1 = client.post(graphql_url, {
        'query': query_categories
    }, content_type='application/json')
    
    if response1.status_code == 200:
        data1 = response1.json()
        print(f"   Résultat: {len(data1.get('data', {}).get('categories', []))} catégories trouvées")
    ...
    response2 = client.post(graphql_url, {
    ...
    response3 = client.post(graphql_url, {
        'query': query_categories
    }, content_type='application/json')
    ...
    if response3.status_code == 200:
        ...
        new_category_found = any(cat['name'] == 'Test Category' for cat in categories_after)
    ...
    response4 = client.post(graphql_url, {
        'query': query_categories
    }, content_type='application/json')
