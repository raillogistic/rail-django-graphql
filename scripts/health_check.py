...
class HealthChecker:
    ...
    def load_config(self, config_file: str) -> Dict:
        ...
        default_config = {
            "database": {
                ...
                "name": os.getenv("DB_NAME", "rail_django_graphql"),
    ...
    def check_database_connectivity(self) -> HealthCheckResult:
        ...
        try:
            ...
            if (
                active_connections
                >= self.config["thresholds"]["critical_db_connections"]
            ):
                status = "critical"
                message = f"Trop de connexions actives: {active_connections}"
            elif (
                active_connections
                >= self.config["thresholds"]["warning_db_connections"]
            ):
                status = "warning"
    ...
    def check_redis_connectivity(self) -> HealthCheckResult:
        ...
        try:
            ...
            r = redis.Redis(
                socket_connect_timeout=timeout, socket_timeout=timeout, **redis_config
            )
    ...
    def check_application_health(self) -> HealthCheckResult:
        ...
        try:
            ...
            return HealthCheckResult(
                "application", status, message, details, duration_ms
            )
            ...
            return HealthCheckResult(
                "application",
                "critical",
                f"Erreur d'accès à l'application: {e}",
                {"error": str(e)},
                duration_ms,
            )
    ...
    def check_graphql_endpoint(self) -> HealthCheckResult:
        ...
        try:
            ...
            payload = {"query": self.config["graphql"]["test_query"]}
    ...
    def check_filesystem(self) -> HealthCheckResult:
        ...
        try:
            ...
            if permission_issues:
                issues.append(
                    f"Problèmes de permissions: {', '.join(permission_issues)}"
                )
            ...
            if issues:
                if missing_dirs or (
                    details.get("disk_space", {}).get("free_gb", float("inf")) < 0.1
                ):
                    status = "critical"
            ...
            return HealthCheckResult(
                "filesystem", status, message, details, duration_ms
            )
            ...
            return HealthCheckResult(
                "filesystem",
                "critical",
                f"Erreur système de fichiers: {e}",
                {"error": str(e)},
                duration_ms,
            )
    ...
    def check_system_resources(self) -> HealthCheckResult:
        ...
        try:
            ...
            return HealthCheckResult(
                "system_resources", status, message, details, duration_ms
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Contrôles de santé pour Django GraphQL Auto"
    )

    parser.add_argument(
        "--config", default="health_check_config.json", help="Fichier de configuration"
    )
    parser.add_argument("--output", help="Fichier de sortie pour les résultats")
    parser.add_argument(
        "--format", choices=["json", "table"], default="table", help="Format de sortie"
    )
    parser.add_argument(
        "--save", action="store_true", help="Sauvegarder les résultats dans un fichier"
    )
    parser.add_argument(
        "--exit-code",
        action="store_true",
        help="Utiliser le code de sortie pour indiquer le statut",
    )
    ...
    if args.save or args.output:
        ...
        if args.format == "json":
            ...
            for check in summary["checks"]:
                status_symbol = {"healthy": "✓", "warning": "⚠", "critical": "✗"}.get(
                    check["status"], "?"
                )

                print(
                    f"{status_symbol} {check['name']:<20} {check['status']:<10} "
                    f"{check['duration_ms']:>6.0f}ms  {check['message']}"
                )

            print(f"{'='*80}")
            print(
                f"Total: {summary['total_checks']} contrôles en {summary['total_duration_ms']:.0f}ms"
            )
            print(
                f"Sains: {summary['healthy_checks']}, "
