...
def debug_input_type():
    ...
    print("\n1. CREATE INPUT TYPE:")
    create_input_type = type_generator.generate_input_type(Post, partial=False, mutation_type='create')
    print(f"Class name: {create_input_type.__name__}")
    
    if hasattr(create_input_type, '_meta') and hasattr(create_input_type._meta, 'fields'):
        print("Fields:")
        for field_name, field in create_input_type._meta.fields.items():
            ...
            print(f"  - {field_name}: {field.type} {'(required)' if is_required else '(optional)'}")
    
    # Generate input type for update
    print("\n2. UPDATE INPUT TYPE:")
    update_input_type = type_generator.generate_input_type(Post, partial=True, mutation_type='update')
    print(f"Class name: {update_input_type.__name__}")
    
    if hasattr(update_input_type, '_meta') and hasattr(update_input_type._meta, 'fields'):
        print("Fields:")
