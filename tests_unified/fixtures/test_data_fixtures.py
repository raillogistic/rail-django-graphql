...
class FixtureTestBookFactory(DjangoModelFactory):
    ...
    format = Faker(
        "random_element", elements=["paperback", "hardcover", "ebook", "audiobook"]
    )
    stock_quantity = Faker("random_int", min=0, max=100)
    rating = Faker(
        "pydecimal", left_digits=1, right_digits=2, positive=True, max_value=5
    )
...
def complete_test_data():
    ...
    for i in range(15):
        book = FixtureTestBookFactory(
            author=random.choice(authors), publisher=random.choice(publishers)
        )
...
def create_complex_relationships() -> Dict[str, Any]:
    ...
    for author in prolific_authors:
        ...
        for _ in range(num_books):
            book = FixtureTestBookFactory(
                author=author, publisher=random.choice(publishers)
            )
...
def create_performance_data(scale: str = "medium") -> Dict[str, Any]:
    ...
    categories = FixtureTestCategoryFactory.create_batch(
        20
    )  # Nombre fixe de catégories
    ...
    for book in books:
        num_reviews = random.randint(max(1, reviews_per_book - 2), reviews_per_book + 2)
        for _ in range(num_reviews):
            review = FixtureTestReviewFactory(book=book)
            reviews.append(review)
...
def hierarchical_categories():
    ...
    sci_fi = FixtureTestCategoryFactory(
        name="Science-Fiction", slug="sci-fi", parent=fiction
    )

    # Sous-catégories de Non-Fiction
    biography = FixtureTestCategoryFactory(
        name="Biographie", slug="biography", parent=non_fiction
    )
    history = FixtureTestCategoryFactory(
        name="Histoire", slug="history", parent=non_fiction
    )
    science = FixtureTestCategoryFactory(
        name="Science", slug="science", parent=non_fiction
    )
...
def books_with_varied_ratings():
    ...
    excellent_book = FixtureTestBookFactory(
        author=author, publisher=publisher, rating=Decimal("4.8")
    )
    good_book = FixtureTestBookFactory(
        author=author, publisher=publisher, rating=Decimal("4.2")
    )
    average_book = FixtureTestBookFactory(
        author=author, publisher=publisher, rating=Decimal("3.5")
    )
    poor_book = FixtureTestBookFactory(
        author=author, publisher=publisher, rating=Decimal("2.1")
    )
    ...
    for book, rating_range in [
        ...
        for _ in range(5):
            FixtureTestReviewFactory(book=book, rating=random.randint(*rating_range))
