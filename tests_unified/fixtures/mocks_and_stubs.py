...
class MockTypeGenerator:
        ...
        mock_type = type(
            ...
            (graphene.ObjectType,),
            {
                ...
                "created_at": graphene.DateTime(
                    required=True, description="Date de création"
                ),
            },
        )
    ...
    def get_field_mapping(self, django_field: models.Field) -> Dict[str, Any]:
        ...
        return field_mappings.get(
            type(django_field), {"type": "String", "required": False}
        )
...
class MockQueryGenerator:
    ...
    def generate_resolver(
        self, model_class: Type[models.Model], query_type: str
    ) -> Callable:
        ...
        if fields:
            ...
            try:
                new_qs._objects.sort(
                    key=lambda obj: getattr(obj, field, ""), reverse=reverse
                )
...
def mock_database_operations(func):
    ...
    def wrapper(*args, **kwargs):
        with patch("django.db.models.QuerySet", MockQuerySet), patch(
            "django.db.models.Manager", MockManager
        ):
