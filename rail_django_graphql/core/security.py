...
class SecuritySettings:
    ...
    allowed_file_types: List[str] = field(default_factory=lambda: [".jpg", ".jpeg", ".png", ".pdf", ".txt"])
    ...
    def from_schema(cls, schema_name: Optional[str] = None) -> "SecuritySettings":
        ...
        django_security_settings = getattr(django_settings, 'RAIL_DJANGO_GRAPHQL', {}).get('security_settings', {})
        ...
        if isinstance(data, str):
            ...
            for pattern in sql_patterns:
                if pattern in lower_data:
                    errors.append(f"Potential SQL injection detected in {path or 'input'}: {pattern}")
