...
class MultipleFileUploadResult(graphene.ObjectType):
    ...
    file_infos = graphene.List(FileInfo, description="Liste des informations sur les fichiers téléchargés")
...
class FileValidator:
    ...
    def validate_file_size(self, file: Union[InMemoryUploadedFile, TemporaryUploadedFile], max_size: int = None) -> bool:
        ...
        if file.size > size_limit:
            raise FileValidationError(f"fichier de taille trop importante: {file.size} bytes (limite: {size_limit} bytes)")
        return True
