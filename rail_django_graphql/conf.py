...
from .defaults import LIBRARY_DEFAULTS, get_library_defaults, get_merged_settings
    ...
    def validate(self) -> Dict[str, str]:
        ...
        for setting, expected_type in type_validations.items():
            ...
            if value is not None and not isinstance(value, expected_type):
                errors[setting] = f"Setting '{setting}' must be of type {expected_type.__name__}"
        ...
        if max_complexity is not None and max_complexity < 1:
            errors['QUERY_SETTINGS.MAX_COMPLEXITY'] = "MAX_COMPLEXITY must be greater than 0"
        
        rate_limit = self.get('SECURITY_SETTINGS.RATE_LIMIT_PER_MINUTE')
        if rate_limit is not None and rate_limit < 1:
            errors['SECURITY_SETTINGS.RATE_LIMIT_PER_MINUTE'] = "RATE_LIMIT_PER_MINUTE must be greater than 0"
...
class SchemaSettingsManager:
    ...
    def get_merged_schema_settings(self, schema_name: str, environment: str = 'development') -> Dict[str, Any]:
        """
...
def get_core_schema_settings(schema_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    return get_setting('CORE_SCHEMA_SETTINGS', {}, schema_name)
