...
class GraphQLAuthenticationMiddleware(MiddlewareMixin):
    ...
    def __init__(self, get_response=None):
        ...
        self.enable_rate_limiting = getattr(settings, 'GRAPHQL_ENABLE_AUTH_RATE_LIMITING', True)
        
        # Cache des utilisateurs pour optimiser les performances
        self.user_cache_timeout = getattr(settings, 'JWT_USER_CACHE_TIMEOUT', 300)  # 5 minutes
    ...
    def _authenticate_jwt_token(self, token: str, request: HttpRequest) -> Optional[User]:
        """
        ...
        try:
            ...
            logger.warning(f"Utilisateur introuvable pour le token JWT: {payload.get('user_id') if 'payload' in locals() else 'unknown'}")
            return None
        except Exception as e:
            logger.error(f"Erreur lors de l'authentification JWT: {e}")
            return None
    
    def _log_authentication_event(self, request: HttpRequest, user: Optional[User], 
                                 event_type: str, auth_method: str) -> None:
        """
        ...
        try:
            ...
            if event_type == 'success':
                ...
                audit_logger.log_token_event(
                    request, user, AuditEventType.TOKEN_INVALID, 
                    success=False, error_message="Token JWT invalide"
                )
            elif event_type == 'inactive_user':
                audit_logger.log_login_attempt(
                    request, user, success=False, 
                    error_message="Compte utilisateur inactif"
                )
            else:
                # Fallback vers l'ancien système de logging
                self._legacy_log_authentication_event(request, user, event_type, auth_method)
    ...
    def _legacy_log_authentication_event(self, request: HttpRequest, user: Optional[User], 
                                       event_type: str, auth_method: str) -> None:
        """
...
class GraphQLRateLimitMiddleware(MiddlewareMixin):
    ...
    def __init__(self, get_response=None):
        ...
        self.login_attempts_window = getattr(settings, 'AUTH_LOGIN_ATTEMPTS_WINDOW', 900)  # 15 minutes
        self.graphql_requests_limit = getattr(settings, 'GRAPHQL_REQUESTS_LIMIT', 100)
        self.graphql_requests_window = getattr(settings, 'GRAPHQL_REQUESTS_WINDOW', 3600)  # 1 heure
    ...
    def process_request(self, request: HttpRequest) -> Optional[HttpResponse]:
        ...
        if self._is_rate_limited(f"graphql_requests_{client_ip}", 
        ...
        if self._is_login_request(request):
            if self._is_rate_limited(f"login_attempts_{client_ip}", 
                                   self.login_attempts_limit, 
                                   self.login_attempts_window):
                return self._create_rate_limit_response("Trop de tentatives de connexion")
    ...
    def _is_login_request(self, request: HttpRequest) -> bool:
        ...
        try:
            ...
            if hasattr(request, 'body') and request.body:
                ...
                return 'login' in query.lower() or 'mutation' in query.lower() and 'login' in query.lower()
        except (json.JSONDecodeError, UnicodeDecodeError, AttributeError):
            pass
        return False
    ...
    def _create_rate_limit_response(self, message: str) -> HttpResponse:
        ...
        try:
            ...
            if hasattr(self, '_current_request'):
                audit_logger.log_rate_limit_exceeded(self._current_request, "GraphQL rate limit")
        except Exception as e:
            logger.error(f"Erreur lors de l'audit de limitation de débit: {e}")
        
        response_data = {
            'errors': [{'message': message, 'code': 'RATE_LIMITED'}]
        }
