...
class RoleBasedPermission(BasePermission):
    ...
    def __init__(self, required_roles: List[str] = None, required_groups: List[str] = None):
...
def create_advanced_user_type():
    ...
    UserType = TypeGenerator.from_model(
        ...
        custom_fields={
            ...
            'is_recent_user': graphene.Boolean(description="Utilisateur récent (moins de 30 jours)"),
        ...
        custom_resolvers={
            'full_name': lambda user, info: f"{user.first_name} {user.last_name}".strip(),
            'initials': lambda user, info: f"{user.first_name[:1]}{user.last_name[:1]}".upper(),
            'post_count': lambda user, info: user.post_set.filter(est_publie=True).count(),
            'comment_count': lambda user, info: user.comment_set.count(),
            'is_recent_user': lambda user, info: (timezone.now() - user.date_joined).days < 30,
            'avatar_url': lambda user, info: f"https://api.dicebear.com/7.x/initials/svg?seed={user.username}",
            'last_activity': lambda user, info: getattr(user, 'last_activity', user.last_login),
    ...
    def resolve_trending_posts(self, info, time_range='week', limit=10):
        ...
        return Post.objects.filter(
            ...
            recent_comments=Count(
                'comments',
                filter=Q(comments__date_commentaire__gte=since)
            )
        ).order_by(
            '-recent_comments',
            '-comment_count',
    ...
    def resolve_search_content(self, info, query, content_types=None, limit=20):
        ...
        if not content_types or 'posts' in content_types:
            post_results = Post.objects.annotate(
                ...
                rank=SearchRank(SearchVector('titre_article', 'contenu_article'), SearchQuery(query))
            ).filter(
        ...
        if not content_types or 'comments' in content_types:
            comment_results = Comment.objects.annotate(
                search=SearchVector('contenu_commentaire'),
                rank=SearchRank(SearchVector('contenu_commentaire'), SearchQuery(query))
            ).filter(
                search=SearchQuery(query)
            ).order_by('-rank')[:limit//2]
...
class CustomLoggingMiddleware(BaseMiddleware):
    ...
    def resolve(self, next, root, info, **args):
        ...
        try:
            ...
            logger.info(f"GraphQL Query Completed: {query_name} in {execution_time:.3f}s")
            
            # Log slow queries
            if execution_time > 1.0:
                logger.warning(f"Slow GraphQL Query: {query_name} took {execution_time:.3f}s")
            ...
            # Log errors
            execution_time = time.time() - start_time
            logger.error(f"GraphQL Query Failed: {query_name} after {execution_time:.3f}s - {str(e)}")
            raise
