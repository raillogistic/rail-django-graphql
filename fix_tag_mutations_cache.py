        ...
        try:
            ...
            for pattern in patterns_to_invalidate:
                try:
                    ...
                    logger.warning(f"⚠️  Erreur d'invalidation du pattern {pattern}: {pattern_error}")
    ...
    def mutate(cls, root, info, id, input=None):
        ...
        try:
            ...
            return UpdateTagWithCacheInvalidation(
                ok=False,
                object=None,
                errors=[f"Tag avec l'ID {id} n'existe pas"]
            )
        except Exception as e:
...
def test_fixed_mutations():
    ...
    try:
        ...
        result = mutation.mutate(
            root=None,
            info=None,
            input={'name': 'TestCacheFixed', 'color': '#FF5733'}
        )
