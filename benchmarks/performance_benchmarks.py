...
class PerformanceBenchmark:
    ...
    def run_load_test(
        self, concurrent_users: List[int] = None, requests_per_user: int = 10
    ):
        """
    ...
    def generate_report(self) -> BenchmarkSummary:
        ...
        unoptimized_results = [
            r for r in successful_results if "unoptimized" in r.scenario
        ]

        performance_improvement = 0.0
        if optimized_results and unoptimized_results:
            avg_optimized = statistics.mean(
                [r.execution_time for r in optimized_results]
            )
            avg_unoptimized = statistics.mean(
                [r.execution_time for r in unoptimized_results]
            )
    ...
    def _create_test_data(self, size: int):
        ...
        for i in range(size // 5):  # 1 auteur pour 5 livres
            author = TestAuthor.objects.create(
                name=f"Auteur {i}", email=f"auteur{i}@test.com"
            )
        ...
        for i in range(size):
            ...
            book = TestBook.objects.create(
                title=f"Livre {i}", author=author, publication_year=2020 + (i % 5)
            )
            books.append(book)

        # Créer un utilisateur pour les avis
        user, _ = User.objects.get_or_create(
            username="testuser", defaults={"email": "test@test.com"}
        )
    ...
    def _run_optimized_query(self):
        ...
        books = (
            TestBook.objects.select_related("author")
            .prefetch_related("testreview_set")
            .all()
        )
        results = []
        for book in books:
...
def run_full_benchmark_suite():
    ...
    try:
        ...
        print(
            f"🚀 Amélioration des performances: {summary.performance_improvement:.1f}%"
        )
