...
def test_multiple_models():
    ...
    cat_vars = {
        "input": {"name": "Original Category", "description": "Original description"}
    }
    ...
    update_cat_vars = {"id": cat_id, "input": {"name": "Updated Category Name"}}
    ...
    update_post_vars = {"id": post_id, "input": {"title": "Updated Post Title Only"}}
    ...
    multi_update_vars = {
        "id": post_id,
        "input": {"title": "Final Title", "is_published": True},
    }
    ...
    if (
        cat_update_result.get("errors")
        or not cat_update_result["data"]["update_category"]["ok"]
    ):
    ...
    if (
        post_update_result.get("errors")
        or not post_update_result["data"]["update_post"]["ok"]
    ):
