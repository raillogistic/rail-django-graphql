    ...
    try:
        ...
        with transaction.atomic():
            ...
            local_client = LocalClient.objects.create(
                raison="Local Client Company", test="Local specific test data"
            )
            print(
                f"Created local client: {local_client.raison} (ID: {local_client.id})"
            )
        ...
        for client_data in clients_data:
            print(
                f"   - ID: {client_data['id']}, Raison: {client_data['raison']}, Type: {client_data['polymorphic_type']}"
            )
        ...
        print(
            f"   - ID: {base_client_data['id']}, Raison: {base_client_data['raison']}, Type: {base_client_data['polymorphic_type']}"
        )
        ...
        print(
            f"   - ID: {local_client_data['id']}, Raison: {local_client_data['raison']}, Type: {local_client_data['polymorphic_type']}"
        )
        ...
        base_in_list = next(
            (c for c in clients_data if c["id"] == str(base_client.id)), None
        )
        local_in_list = next(
            (c for c in clients_data if c["id"] == str(local_client.id)), None
        )
        ...
        if base_in_list and base_in_list["polymorphic_type"] == "Client":
            ...
            print(
                f"❌ Base client has incorrect polymorphic_type: {base_in_list['polymorphic_type'] if base_in_list else 'Not found'}"
            )
            return False

        if local_in_list and local_in_list["polymorphic_type"] == "LocalClient":
            print(
                "✅ Local client has correct polymorphic_type 'LocalClient' in list query"
            )
        else:
            print(
                f"❌ Local client has incorrect polymorphic_type: {local_in_list['polymorphic_type'] if local_in_list else 'Not found'}"
            )
            return False

        if base_client_data["polymorphic_type"] == "Client":
            print(
                "✅ Base client has correct polymorphic_type 'Client' in single query"
            )
        else:
            print(
                f"❌ Base client has incorrect polymorphic_type in single query: {base_client_data['polymorphic_type']}"
            )
            return False

        if local_client_data["polymorphic_type"] == "LocalClient":
            print(
                "✅ Local client has correct polymorphic_type 'LocalClient' in single query"
            )
        else:
            print(
                f"❌ Local client has incorrect polymorphic_type in single query: {local_client_data['polymorphic_type']}"
            )
            return False
