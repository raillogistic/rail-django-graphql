...
def test_polymorphic_mutation_schema():
    ...
    try:
        ...
        polymorphic_in_inputs = any(field in ['polymorphic_ctype', 'client_ptr'] for field in client_input_fields)
        
        if not polymorphic_in_inputs:
            print("   ✅ SUCCESS: No polymorphic fields found in Client input types")
        else:
            print("   ❌ FAILED: Polymorphic fields still present in Client input types")
        ...
        if mutation_type:
            ...
            client_mutations = [name for name in mutation_fields.keys() if 'client' in name.lower()]
            ...
            if 'createClient' in mutation_fields:
                ...
                if input_arg:
                    ...
                    if hasattr(input_type, 'fields'):
                        ...
                        polymorphic_in_create = any(field in ['polymorphic_ctype', 'client_ptr'] for field in input_field_names)
                        if not polymorphic_in_create:
                            print("   ✅ SUCCESS: createClient mutation excludes polymorphic fields")
                        else:
                            print("   ❌ FAILED: createClient mutation includes polymorphic fields")
