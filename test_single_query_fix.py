...
def test_single_query_fix():
    ...
    local_client = LocalClient.objects.create(raison="Test Local Client for Fix", test="Local Info")
    
    # Create a category first for the post
    category = Category.objects.create(name="Test Category", description="Test category for fix")
    post = Post.objects.create(title="Test Post for Fix", content="Testing single query fix", category=category)
    ...
    if result.errors:
        ...
        if client_data:
            ...
            if client_data['polymorphic_type'] != 'Client':
                print(f"   ⚠️  Expected 'Client', got '{client_data['polymorphic_type']}'")
        else:
    ...
    if result.errors:
        ...
        if local_client_data:
            ...
            if local_client_data['polymorphic_type'] != 'LocalClient':
                print(f"   ⚠️  Expected 'LocalClient', got '{local_client_data['polymorphic_type']}'")
        else:
