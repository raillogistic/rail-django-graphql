...
from rail_django_graphql.extensions.media import (
    MediaManager,
    ImageProcessor,
    ThumbnailSize,
    MediaInfo,
...
class MediaInfoTest(TestCase):
    ...
    def test_media_info_aspect_ratio(self):
        ...
        media_info = MediaInfo(filename="test.jpg", width=800, height=600)
...
class ImageProcessorTest(TestCase):
    ...
    def test_resize_image_proportional(self):
        ...
        resized_file = self.processor.resize_image(
            uploaded_file, max_width=400, max_height=300
        )
    ...
    def test_resize_image_no_upscaling(self):
        ...
        resized_file = self.processor.resize_image(
            uploaded_file, max_width=800, max_height=600
        )
    ...
    def test_generate_thumbnail_small(self):
        ...
        thumbnail_file = self.processor.generate_thumbnail(
            uploaded_file, ThumbnailSize.SMALL
        )
    ...
    def test_generate_thumbnail_medium(self):
        ...
        thumbnail_file = self.processor.generate_thumbnail(
            uploaded_file, ThumbnailSize.MEDIUM
        )
    ...
    def test_generate_thumbnail_large(self):
        ...
        thumbnail_file = self.processor.generate_thumbnail(
            uploaded_file, ThumbnailSize.LARGE
        )
...
class CDNManagerTest(TestCase):
    ...
    def test_get_optimized_url(self):
        ...
        optimized_url = self.cdn_manager.get_optimized_url(
            file_path, width=300, height=200, quality=80
        )
...
class MediaManagerTest(TestCase):
    ...
    @patch("rail_django_graphql.extensions.media.LocalStorageBackend")
    ...
    def test_optimize_existing_media(self):
        ...
        with patch(
            "rail_django_graphql.extensions.media.LocalStorageBackend"
        ) as mock_storage_backend:
            mock_backend = Mock()
            ...
            with patch("django.core.files.storage.default_storage.open") as mock_open:
                ...
                result = self.media_manager.optimize_existing_media(
                    "original_image.jpg", quality=70, max_width=600, max_height=400
                )
...
class MediaIntegrationTest(TestCase):
    ...
    @patch("rail_django_graphql.extensions.media.LocalStorageBackend")
    @patch("rail_django_graphql.extensions.media.CDNManager")
    ...
    def test_batch_thumbnail_generation(self, mock_storage_backend):
        ...
        for image in images:
            media_info = self.media_manager.process_and_save_image(
                image, generate_thumbnails=True
            )
