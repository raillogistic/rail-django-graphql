...
from rail_django_graphql.extensions.optimization import (
    QueryOptimizer,
    QueryAnalyzer,
    PerformanceMonitor,
...
from rail_django_graphql.extensions.caching import (
    GraphQLCacheManager,
    CacheConfig,
    get_cache_manager,
    cache_query,
...
from rail_django_graphql.middleware.performance import (
...
class TestQueryOptimization(TestCase):
    ...
    def setUp(self):
        ...
        self.author1 = PerformanceTestAuthor.objects.create(
            name="Auteur 1", email="auteur1@test.com"
        )
        self.author2 = PerformanceTestAuthor.objects.create(
            name="Auteur 2", email="auteur2@test.com"
        )
    ...
    def test_n_plus_one_prevention(self):
        ...
        with patch.object(self.optimizer, "optimize_queryset") as mock_optimize:
            mock_optimize.return_value = PerformanceTestBook.objects.select_related(
                "author"
            ).all()
    ...
    def test_queryset_optimization(self):
        ...
        optimized_queryset = self.optimizer.optimize_queryset(
            queryset, mock_info, PerformanceTestBook
        )
...
class TestCachingSystem(TestCase):
    ...
    def test_cache_invalidation(self):
        ...
        self.cache_manager.set_field_value(
            "PerformanceTestBook", "title", 1, "Title", 1
        )

        # Vérifier qu'ils sont en cache
        self.assertIsNotNone(self.cache_manager.get_query_result("query1", {}, 1))
        self.assertIsNotNone(
            self.cache_manager.get_field_value("PerformanceTestBook", "title", 1, 1)
        )
...
class TestPerformanceMonitoring(TestCase):
    ...
    def test_performance_metrics_collection(self):
        ...
        from rail_django_graphql.middleware.performance import RequestMetrics
    ...
    @patch("rail_django_graphql.middleware.performance.logger")
    def test_performance_alerts(self, mock_logger):
        ...
        alerts = [
            alert
            for alert in self.aggregator.alerts_history
...
class TestIntegrationPerformance(TestCase):
    ...
    def setUp(self):
        ...
        # Créer des données de test
        self.author = PerformanceTestAuthor.objects.create(
            name="Test Author", email="test@test.com"
        )
        self.books = [
            PerformanceTestBook.objects.create(
                title=f"Book {i}", author=self.author, publication_year=2020 + i
            )
            for i in range(10)
        ]
    ...
    def test_performance_benchmarks(self):
        ...
        authors = [
            PerformanceTestAuthor.objects.create(
...
class TestPerformanceOptimizationPytest:
    ...
    @patch("rail_django_graphql.extensions.optimization.time.time")
