    ...
    try:
        ...
        for expected_field in expected_fields:
            if expected_field not in model_fields:
                errors.append(f"Champ manquant dans {model_class.__name__}: {expected_field}")
...
def compare_data_structures(data1: Any, data2: Any, ignore_keys: List[str] = None) -> bool:
...
class TestLogCapture:
    ...
    def __init__(self, logger_name: str = None, level: int = logging.DEBUG):
        self.logger_name = logger_name or 'django_graphql_auto'
        self.level = level
        self.logs = []
        self.handler = None
        self.logger = None
