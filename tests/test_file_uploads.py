...
from rail_django_graphql.generators.file_uploads import (
    FileUploadGenerator,
    FileInfo,
    FileUploadResult,
...
from rail_django_graphql.extensions.media import MediaProcessingError
from rail_django_graphql.extensions.virus_scanner import (
    VirusScanner,
    ScanResult,
    ThreatDetected,
    VirusScanError,
)
from rail_django_graphql.core.settings import GraphQLAutoConfig
...
def test_file_info_creation():
    ...
    file_info = FileInfo(
        name="test.txt", size=1024, content_type="text/plain", extension=".txt"
    )
...
def test_validate_file_size_success():
    ...
    file_info = FileInfo(
        name="small.txt", size=100, content_type="text/plain", extension=".txt"
    )
...
def test_validate_file_size_failure():
    ...
    file_info = FileInfo(
        name="large.txt", size=2048, content_type="text/plain", extension=".txt"
    )
...
def test_validate_file_type_success():
    ...
    file_info = FileInfo(
        name="document.txt", size=100, content_type="text/plain", extension=".txt"
    )
...
def test_validate_file_type_failure():
    ...
    file_info = FileInfo(
        name="image.jpg", size=100, content_type="image/jpeg", extension=".jpg"
    )
...
def test_validate_file_extension_failure():
    ...
    file_info = FileInfo(
        name="script.py", size=100, content_type="text/x-python", extension=".py"
    )
...
@pytest.mark.django_db
@patch("rail_django_graphql.generators.file_uploads.VirusScanner")
def test_process_single_file_upload_success(mock_scanner_class):
    ...
    result = generator.process_single_file_upload(
        uploaded_file, TestFileModel, user, additional_fields={"name": "Test Upload"}
    )
...
def test_process_multiple_file_upload(mock_scanner_class):
    ...
    result = generator.process_multiple_file_upload(
        files, TestFileModel, user, additional_fields={"name": "Multiple Upload Test"}
    )
...
def test_complete_file_upload_workflow():
    ...
    with patch(
        "rail_django_graphql.generators.file_uploads.VirusScanner"
...
def test_image_upload_and_processing(mock_image_open):
    ...
    with patch(
        ...
        result = generator.process_single_file_upload(
            uploaded_file, TestFileModel, user, additional_fields={"name": "Image Test"}
        )
