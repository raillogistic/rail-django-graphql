...
from django_graphql_auto.generators.mutations import (
    MutationGenerator,
    MutationInfo,
    InputTypeGenerator,
    create_mutation_class,
    get_mutation_resolver
)
...
from tests.models import TestProduct, TestProductCategory, TestOrder, TestOrderItem
...
class TestMutationGenerator(TestCase):
    ...
    def test_initialization(self):
        ...
        generator = MutationGenerator(self.type_generator, self.input_generator)
        self.assertIsNotNone(generator)
        self.assertEqual(generator.type_generator, self.type_generator)
        self.assertEqual(generator.input_generator, self.input_generator)
    ...
    def test_generate_create_mutation(self):
        ...
        # Générer la mutation de création pour TestProduct
    ...
    def test_generate_update_mutation(self):
        ...
        # Générer la mutation de mise à jour pour TestProduct
        update_mutation = self.mutation_generator.generate_update_mutation(TestProduct)
    ...
    def test_generate_delete_mutation(self):
        ...
        # Générer la mutation de suppression pour TestProduct
        delete_mutation = self.mutation_generator.generate_delete_mutation(TestProduct)
    ...
    def test_generate_business_method_mutations(self):
        ...
        business_mutations = self.mutation_generator.generate_business_method_mutations(TestProduct)
        ...
        expected_methods = ['augmenter_stock', 'diminuer_stock', 'calculer_valeur_stock']
    ...
    def test_custom_mutation_integration(self):
        ...
        config_mock.custom_mutations = {
            'TestProduct': {
                'custom_action': CustomProductMutation
            }
        }
    ...
    def test_mutation_hooks(self):
        ...
        config_mock.pre_mutation_hooks = {
            'TestProduct': {
                'create': [Mock()]
            }
        }
        config_mock.post_mutation_hooks = {
            'TestProduct': {
                'create': [Mock()]
            }
        }
    ...
    @patch('django_graphql_auto.generators.mutations.logger')
...
class TestInputTypeGenerator(TestCase):
    ...
    def test_input_type_with_relationships(self):
        ...
        # Générer le type d'entrée pour TestOrder (qui a des relations)
        order_input = self.input_generator.generate_input_type(TestOrder, 'create')
        
        # Vérifier que le type d'entrée est généré
        self.assertIsNotNone(order_input)
        
        # Vérifier que les relations sont gérées
        if hasattr(order_input, '_meta'):
            fields = order_input._meta.fields
            # Les clés étrangères doivent être présentes
            self.assertIn('client_commande', fields)


class TestMutationInfo(TestCase):
    """Tests pour la classe MutationInfo."""
    
    def test_mutation_info_creation(self):
        """Test la création d'un objet MutationInfo."""
        mutation_class = Mock()
        ...
        self.assertEqual(mutation_info.name, 'create_product')
        self.assertEqual(mutation_info.mutation_type, 'create')
        self.assertEqual(mutation_info.model, TestProduct)
        self.assertEqual(mutation_info.mutation_class, mutation_class)
        self.assertIsNotNone(mutation_info.input_type)
        self.assertEqual(mutation_info.description, "Create a new product")
...
class TestMutationGeneratorIntegration:
    ...
    def test_business_method_integration(self):
        ...
        business_mutations = mutation_generator.generate_business_method_mutations(TestProduct)
...
class TestAdvancedMutationFeatures(TestCase):
    ...
    def test_nested_mutations(self):
        ...
        nested_mutation = generator.generate_nested_mutation(TestOrder, ['items_commande'])
    ...
    def test_mutation_middleware_integration(self):
        ...
        config_mock.mutation_middlewares = [
            'django_graphql_auto.middleware.ValidationMiddleware',
            'django_graphql_auto.middleware.LoggingMiddleware'
        ]
