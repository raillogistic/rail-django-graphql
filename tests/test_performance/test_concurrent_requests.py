...
class ConcurrencyTester:
    ...
    def execute_query_thread(self, query: str, thread_id: int, iterations: int = 1):
        ...
        with self.lock:
            self.thread_results[thread_id] = {
                ...
                'success_rate': len(thread_results) / (len(thread_results) + len(thread_errors))
            }
    ...
    def execute_concurrent_queries(
        ...
        for i in range(num_threads):
            thread = threading.Thread(
                ...
                args=(query, i, iterations_per_thread)
            )
    ...
    def execute_mixed_workload(
        ...
        def worker():
            ...
            while time.time() - thread_start < duration_seconds:
                ...
                try:
                    ...
                    with self.lock:
                        results.append({
                            'query': selected_query[:50] + '...',  # Tronquer pour l'affichage
        ...
        return {
            ...
            'successful_queries': len(successful_queries),
            'failed_queries': len(errors),
            'success_rate': len(successful_queries) / (len(results) + len(errors)) if results or errors else 0,
            'queries_per_second': (len(results) + len(errors)) / total_time if total_time > 0 else 0,
            'results': results,
            'errors': errors
        }
...
class TestConcurrentRequests(TransactionTestCase):
    ...
    def _create_test_data(self):
        ...
        for i in range(10):
            resource = TestSharedResource.objects.create(
                nom_ressource=f"Resource {i}",
                valeur_partagee=i * 10,
                nombre_acces=0
            )
    ...
    def test_concurrent_write_mutations(self):
        ...
        def execute_mutation_thread(thread_id: int):
            ...
            model_id = self.concurrent_models[thread_id % len(self.concurrent_models)].id
    ...
    def test_cache_concurrency(self):
        ...
        self.assertGreater(stats['success_rate'], 0.95)
        self.assertLess(stats['execution_time_stats']['mean'], 0.5)  # Cache doit accélérer
        
        # Pas d'erreurs de concurrence du cache
        cache_errors = [e for e in stats['errors'] if 'cache' in e.get('error', '').lower()]
    ...
    def test_error_handling_under_concurrency(self):
        ...
        error_rate = len(stats['errors']) / stats['total_queries'] if stats['total_queries'] > 0 else 0
        self.assertLess(error_rate, 0.5)  # Moins de 50% d'erreurs (les valides doivent passer)
    ...
    def test_scalability_increasing_load(self):
        ...
        for i, result in enumerate(performance_results):
            ...
            if i > 0:
                ...
                qps_ratio = result['qps'] / prev_result['qps'] if prev_result['qps'] > 0 else 1
