...
def test_additional_models():
    ...
    result = client.execute(update_comment_mutation, variables={"id": comment_id, **update_comment_vars})
    print(f"Comment partial update: {result}")
    
    # Test 3: Multiple field update
    multi_update_vars = {
        "input": {
            "content": "Final comment content",
            "is_approved": True
        }
    }
    
    result = client.execute(update_comment_mutation, variables={"id": comment_id, **multi_update_vars})
    ...
    result = client.execute(update_profile_mutation, variables={"id": profile_id, **update_profile_vars})
    print(f"Profile partial update: {result}")
    
    # Test 6: Multiple field update
    multi_profile_vars = {
        "input": {
            "bio": "Final bio content",
            "birth_date": "1985-05-15"
        }
    }
    
    result = client.execute(update_profile_mutation, variables={"id": profile_id, **multi_profile_vars})
