...
class PropertyFilteringTestSuite:
    ...
    def test_property_detection(self):
        ...
        assert (
            "uppercase_raison" in properties
        ), "uppercase_raison property not detected"

        prop_info = properties["uppercase_raison"]
        assert (
            prop_info.return_type == str
        ), f"Expected str, got {prop_info.return_type}"
    ...
    def test_filter_generation(self):
        ...
        property_filter_names = [
            name for name in filters.keys() if "uppercase_raison" in name
        ]
    ...
    def test_icontains_filtering(self):
        ...
        if len(clients) >= 1:
            ...
            if len(tech_clients) >= 1:
                print(
                    f"Found {len(clients)} clients with case-insensitive contains filtering"
                )
                return True
        return False
