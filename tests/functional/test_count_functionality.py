...
def test_count_field_generation():
    ...
    reverse_count_fields = [
        field for field in product_fields.keys() if "_count" in field
    ]
...
def test_count_functionality_with_data():
    ...
    try:
        ...
        category = TestProductCategory.objects.create(
            nom_categorie="Electronics", description_categorie="Electronic products"
        )
        ...
        order1 = TestOrder.objects.create(
            numero_commande="ORD001", client_commande=user, statut_commande="EN_ATTENTE"
        )

        order2 = TestOrder.objects.create(
            numero_commande="ORD002", client_commande=user, statut_commande="CONFIRMEE"
        )
        ...
        product1_order_count = TestOrder.objects.filter(
            produits_commande=product1
        ).count()
