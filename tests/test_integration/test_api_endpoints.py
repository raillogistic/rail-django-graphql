...
from django_graphql_auto.middleware.authentication import AuthenticationMiddleware
from django_graphql_auto.middleware.permissions import PermissionMiddleware
from django_graphql_auto.middleware.rate_limiting import RateLimitingMiddleware
from django_graphql_auto.middleware.logging import LoggingMiddleware
...
TEST_GRAPHQL_SETTINGS = {
    'GRAPHENE': {
        ...
        'MIDDLEWARE': [
            'django_graphql_auto.middleware.authentication.AuthenticationMiddleware',
            'django_graphql_auto.middleware.permissions.PermissionMiddleware',
            'django_graphql_auto.middleware.rate_limiting.RateLimitingMiddleware',
            'django_graphql_auto.middleware.logging.LoggingMiddleware',
        ],
    },
    'DJANGO_GRAPHQL_AUTO': {
        ...
        'PERMISSION_CLASSES': [
            ...
            'django_graphql_auto.permissions.HasModelPermission',
        ],
        'RATE_LIMITING': {
            'ENABLED': True,
...
class TestAPIEndpointsIntegration(TestCase):
    ...
    def test_error_handling_endpoint(self):
        ...
        if 'data' in response_data and response_data['data']:
            ...
            if create_result:
                ...
                self.assertTrue(
                    not create_result.get('success', True) or 
    ...
    def test_rate_limiting_endpoint(self):
        ...
        rate_limited = any(
            response.status_code == 429 for response in responses
        )
    ...
    def test_cors_headers_endpoint(self):
        ...
        cors_configured = any(
            header in response for header in cors_headers
        )
