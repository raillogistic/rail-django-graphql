...
def test_complete_solution():
    ...
    try:
        ...
        update_result = update_mutation.mutate(
            None, None, id=test_tag.pk, input=update_input
        )

        if update_result and update_result.tag:
            print(
                f"✅ Tag mis à jour: {update_result.tag.name} -> {update_result.tag.color}"
            )
    ...
    try:
        ...
        if (
            cache.get("graphql_all_tags") is None
            and cache.get("gql_query_tags") is None
        ):
    ...
    try:
        ...
        if new_tag_result and new_tag_result.tag:
            ...
            if cached_tags is None:
                ...
                print(
                    f"✅ SOLUTION FONCTIONNE: Cache invalidé, {len(fresh_tags)} tags récupérés"
                )
                print(
                    "✅ Le nouveau tag sera visible immédiatement sans rafraîchissement!"
                )
            else:
                print(
                    "❌ PROBLÈME PERSISTE: Cache non invalidé, ancien résultat retourné"
                )
                print(
                    "❌ L'utilisateur devra rafraîchir la page pour voir le nouveau tag"
                )
    ...
    print(
        '   mutation { createTag(input: {name: "Test", color: "#FF0000"}) { tag { id name color } } }'
    )
