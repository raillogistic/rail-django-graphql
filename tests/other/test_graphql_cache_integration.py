...
def test_graphql_query_caching():
    ...
    try:
        ...
        print(
            f"✅ Première requête réussie - {len(data1.get('data', {}).get('categories', []))} catégories"
        )
        ...
        print(
            f"📝 Valeur en cache avant mutation: {'✅' if cached_value_before else '❌'} - {cached_value_before}"
        )
...
def test_graphql_mutation_cache_invalidation():
    ...
    print(
        f"   test_mutation_cache: {'✅' if cached_before else '❌'} - {cached_before}"
    )
    print(
        f"   graphql_categories_cache: {'✅' if graphql_cached_before else '❌'} - {graphql_cached_before}"
    )
    ...
    try:
        ...
        if created_category:
            print(
                f"✅ Catégorie créée via GraphQL: {created_category['name']} (ID: {created_category['id']})"
            )
        else:
        ...
        print(
            f"   test_mutation_cache: {'❌ INVALIDÉ' if not cached_after else '✅ PRÉSENT'} - {cached_after}"
        )
        print(
            f"   graphql_categories_cache: {'❌ INVALIDÉ' if not graphql_cached_after else '✅ PRÉSENT'} - {graphql_cached_after}"
        )
...
def test_direct_model_operations():
    ...
    print(
        f"   model_operations_cache: {'✅' if model_cached_before else '❌'} - {model_cached_before}"
    )
    ...
    print(
        f"   direct_test_cache: {'❌ INVALIDÉ' if not cached_after else '✅ PRÉSENT'} - {cached_after}"
    )
    print(
        f"   model_operations_cache: {'❌ INVALIDÉ' if not model_cached_after else '✅ PRÉSENT'} - {model_cached_after}"
    )
...
def run_graphql_integration_tests():
    ...
    print(
        f"🌐 Serveur GraphQL: {'✅ DISPONIBLE' if server_available else '❌ INDISPONIBLE'}"
    )
    print(
        f"🔍 Requêtes GraphQL: {'✅ OK' if query_caching_ok else '❌ ÉCHEC' if server_available else '⏭️ IGNORÉ'}"
    )
    print(
        f"🔄 Mutations GraphQL: {'✅ OK' if mutation_invalidation_ok else '❌ ÉCHEC' if server_available else '⏭️ IGNORÉ'}"
    )
    print(
        f"🔧 Opérations directes: {'✅ OK' if direct_invalidation_ok else '❌ ÉCHEC'}"
    )

    # Évaluer le succès global
    if server_available:
        overall_success = (
            query_caching_ok and mutation_invalidation_ok and direct_invalidation_ok
        )
    else:
