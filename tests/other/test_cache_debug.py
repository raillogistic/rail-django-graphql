...
def test_cache_behavior():
    ...
    print(
        f"Première requête - Catégories trouvées: {len(result1.get('data', {}).get('categories', []))}"
    )
    ...
    print(
        f"Requête après création - Catégories trouvées: {len(categories_after_create)}"
    )

    # Vérifier si la nouvelle catégorie est présente
    new_category_found = any(
        cat["name"] == "Catégorie Test Cache" for cat in categories_after_create
    )
    ...
    new_category_found_after_delay = any(
        cat["name"] == "Catégorie Test Cache" for cat in categories_after_delay
    )
    ...
    new_category_found_after_clear = any(
        cat["name"] == "Catégorie Test Cache" for cat in categories_after_clear
    )
...
def main():
    ...
    print("=" * 50)
    print(
        f"Cache des catégories: {'✅ Fonctionne' if cache_working else '❌ Ne fonctionne pas'}"
    )
    print(
        f"Cache des tags: {'✅ Fonctionne' if tag_cache_working else '❌ Ne fonctionne pas'}"
    )
