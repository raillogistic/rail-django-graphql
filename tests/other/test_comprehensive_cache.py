...
def test_post_cache_invalidation():
    ...
    category = Category.objects.create(
        name="Test Category for Post",
        description="Catégorie pour test post"
    )
...
def test_comment_cache_invalidation():
    ...
    category = Category.objects.create(
        name="Test Category for Comment",
        description="Catégorie pour test commentaire"
    )
...
def main():
    ...
    try:
        ...
        if success_count == total_tests:
            logger.info("🎉 SUCCÈS COMPLET: L'invalidation du cache fonctionne pour tous les modèles!")
            logger.info("✅ Recommandation: Testez maintenant manuellement dans GraphiQL")
            logger.info("   1. Créez une catégorie, un tag, un post ou un commentaire")
            logger.info("   2. Vérifiez qu'ils apparaissent immédiatement dans les listes")
            logger.info("   3. Modifiez-les et vérifiez les changements")
            logger.info("   4. Supprimez-les et vérifiez qu'ils disparaissent")
        else:
            logger.error(f"❌ ÉCHEC PARTIEL: {success_count}/{total_tests} tests réussis")
