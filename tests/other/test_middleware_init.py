...
def test_bulk_operations_with_middleware():
    ...
    categories_to_create = [
        Category(
            name=f"Bulk Middleware Category {i}", description=f"Catégorie en lot {i}"
        )
        for i in range(3)
    ]
...
def test_different_models_with_middleware():
    ...
    tag = Tag.objects.create(
        name=f"Test Middleware Tag {datetime.now().strftime('%H%M%S')}"
    )
...
def check_signal_connections():
    ...
    for receiver in post_save_receivers:
        if "cache_invalidation" in str(
            receiver
        ) or "GraphQLCacheInvalidationMiddleware" in str(receiver):
...
def run_middleware_test():
    ...
    try:
        ...
        results.append(
            (
                "Opérations CRUD avec middleware",
                test_cache_invalidation_with_middleware(),
            )
        )
        ...
        results.append(
            (
                "Opérations en lot avec middleware",
                test_bulk_operations_with_middleware(),
            )
        )
        ...
        results.append(
            (
                "Différents modèles avec middleware",
                test_different_models_with_middleware(),
            )
        )
