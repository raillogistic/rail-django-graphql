...
def test_graphql_cache_invalidation():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "errors" not in result:
                ...
                if mutation_result["ok"]:
                    ...
                    remaining_keys = (
                        list(cache._cache.keys()) if hasattr(cache, "_cache") else []
                    )

                    if len(remaining_keys) < len(initial_keys):
                        print(
                            f"   ✅ Cache invalidé: {len(initial_keys)} -> {len(remaining_keys)} clés"
                        )
                        cache_invalidated = True
                    else:
                        print(
                            f"   ❌ Cache non invalidé: {len(initial_keys)} -> {len(remaining_keys)} clés"
                        )
                        cache_invalidated = False
                else:
                    print(
                        f"   ❌ Mutation échouée: {mutation_result.get('errors', 'Erreur inconnue')}"
                    )
                    cache_invalidated = False
            else:
        ...
        Category.objects.create(
            name="TestCacheCategory", description="Test category for cache invalidation"
        )
        ...
        if len(remaining_keys) < len(initial_keys):
            print(
                f"   ✅ Cache invalidé (opération directe): {len(initial_keys)} -> {len(remaining_keys)} clés"
            )
            cache_invalidated = True
        else:
            print(
                f"   ❌ Cache non invalidé (opération directe): {len(initial_keys)} -> {len(remaining_keys)} clés"
            )
            cache_invalidated = False
