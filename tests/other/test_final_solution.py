...
def test_final_solution():
    ...
    if result3 and 'data' in result3:
        ...
        if new_count > initial_count:
            ...
            if found_new_category:
                print(f"✅ La nouvelle catégorie '{test_name}' est visible immédiatement")
            else:
                print(f"⚠️ La nouvelle catégorie '{test_name}' n'est pas trouvée dans la liste")
    ...
    if result_tag and 'data' in result_tag and result_tag['data']['create_tag']['ok']:
        ...
        if result_tags_after and 'data' in result_tags_after:
            ...
            if new_tags_count > initial_tags_count:
                print("🎉 SUCCÈS! Le middleware a invalidé le cache pour les tags aussi!")
    ...
    if result_custom and 'data' in result_custom:
        ...
        if custom_result and custom_result.get('success'):
            print(f"✅ Mutation personnalisée réussie: {custom_result['category']['name']}")
            ...
            if result_after_custom and 'data' in result_after_custom:
                ...
                found_custom = any(cat['name'] == custom_name for cat in categories_after_custom)
                
                if found_custom:
                    print("🎉 SUCCÈS! Le middleware fonctionne aussi avec les mutations personnalisées!")
                else:
                    print("⚠️ La mutation personnalisée n'a pas déclenché l'invalidation du cache")
        else:
    ...
    try:
        ...
        if db_custom_category:
            print(f"✅ Catégorie personnalisée trouvée en base: {db_custom_category.name}")
        else:
