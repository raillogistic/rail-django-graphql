...
def test_filter_consistency():
    ...
    # Create users
    user1, _ = AuthUser.objects.get_or_create(
    ...
    Comment.objects.filter(
        content__contains="Filter test"
    ).delete()  # Clean up previous tests

    comment1 = Comment.objects.create(
        post=post1, author=user1, content="Filter test comment 1", is_approved=True
    )
    comment2 = Comment.objects.create(
        post=post1, author=user2, content="Filter test comment 2", is_approved=False
    )
    comment3 = Comment.objects.create(
        post=post2, author=user1, content="Filter test comment 3", is_approved=True
    )

    print(
        f"Created {Comment.objects.filter(content__contains='Filter test').count()} test comments"
    )
    ...
    if result.errors:
        ...
        print(
            f"Found {len(approved_comments_paginated)} approved comments in paginated query"
        )
    ...
    if result.errors:
        ...
        print(
            f"Found {len(user1_comments_paginated)} comments by user1 in paginated query"
        )
        print(f"Total count: {page_data['page_info']['total_count']}")
    ...
    if result.errors:
        ...
        print(
            f"Found {len(multi_author_comments)} comments by multiple authors in list query"
        )
    ...
    if result.errors:
        ...
        print(
            f"Found {len(multi_author_comments_paginated)} comments by multiple authors in paginated query"
        )
        print(f"Total count: {page_data['page_info']['total_count']}")
    ...
    if result.errors:
        ...
        print(
            f"Found {len(filtered_comments_paginated)} comments containing 'Filter test' in paginated query"
        )
        print(f"Total count: {page_data['page_info']['total_count']}")
