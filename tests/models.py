...
from django_graphql_auto.decorators import business_logic as business_method
...
class TestAccount(models.Model):
    ...
    @business_method
    def effectuer_virement(self, compte_destinataire: 'TestAccount', montant: float) -> bool:
    ...
    def clean(self):
        ...
        if self.solde_compte < 0 and self.type_compte != 'COURANT':
            raise ValidationError("Seuls les comptes courants peuvent avoir un solde négatif")
...
class TestTransaction(models.Model):
    ...
    description_transaction = models.TextField(
        blank=True,
        verbose_name="Description de la transaction"
    )
    ...
    def clean(self):
        ...
        if self.type_transaction == 'VIREMENT' and not self.compte_destination:
            raise ValidationError("Un compte de destination est requis pour les virements")
...
class TestOrder(models.Model):
    ...
    def calculer_total(self) -> float:
        ...
        return sum(item.quantite_item * item.prix_unitaire for item in self.items_commande.all())
...
class TestOrderItem(models.Model):
    ...
    quantite_item = models.PositiveIntegerField(
        verbose_name="Quantité de l'item"
    )
...
class TestFileModel(models.Model):
    ...
    fichier_telecharge = models.FileField(
        ...
        verbose_name="Fichier téléchargé"
    )
...
class TestUser(models.Model):
    ...
    score_reputation = models.FloatField(
        default=0.0,
        verbose_name="Score de réputation"
    )
...
class TestMemoryModel(models.Model):
    ...
    donnees_volumineuses = models.TextField(
        verbose_name="Données volumineuses"
    )
    nombre_entier = models.IntegerField(
        default=0,
        verbose_name="Nombre entier"
    )
    donnees_json = models.TextField(
        default='{}',
...
class TestRelatedModel(models.Model):
    ...
    nom_relation = models.CharField(
        max_length=100,
        verbose_name="Nom de la relation"
    )
    donnees_relation = models.TextField(
        verbose_name="Données de la relation"
    )
...
class BenchmarkTestAuthor(models.Model):
    ...
    prenom_auteur = models.CharField(
        max_length=100,
        verbose_name="Prénom de l'auteur"
    )
    email_auteur = models.EmailField(
        unique=True,
        verbose_name="E-mail de l'auteur"
    )
    date_naissance = models.DateField(
        null=True,
        blank=True,
    ...
    biographie_auteur = models.TextField(
        blank=True,
        verbose_name="Biographie de l'auteur"
    )
    nombre_livres = models.IntegerField(
        default=0,
        verbose_name="Nombre de livres"
    )
    est_actif = models.BooleanField(
        default=True,
        verbose_name="Est actif"
...
class BenchmarkTestBook(models.Model):
    ...
    prix_livre = models.DecimalField(
        max_digits=8,
        decimal_places=2,
        verbose_name="Prix du livre"
    )
    description_livre = models.TextField(
        blank=True,
        verbose_name="Description du livre"
    )
...
class BenchmarkTestReview(models.Model):
    ...
    nom_revieweur = models.CharField(
        max_length=100,
        verbose_name="Nom du revieweur"
    )
    email_revieweur = models.EmailField(
        verbose_name="E-mail du revieweur"
    )
    note_avis = models.IntegerField(
        choices=[(i, str(i)) for i in range(1, 6)],
        verbose_name="Note de l'avis"
    )
    commentaire_avis = models.TextField(
        verbose_name="Commentaire de l'avis"
    )
...
class TestPerformanceModel(models.Model):
    ...
    name = models.CharField(
        max_length=100,
        verbose_name="Nom du test de performance"
    )
    description = models.TextField(
        blank=True,
        verbose_name="Description du test"
    )
...
class TestConcurrencyModel(models.Model):
    ...
    name = models.CharField(
        max_length=100,
        verbose_name="Nom du test de concurrence"
    )
    thread_count = models.IntegerField(
        default=1,
        verbose_name="Nombre de threads"
    )
    concurrent_operations = models.IntegerField(
        default=1,
        verbose_name="Opérations concurrentes"
    )
    success_count = models.IntegerField(
        default=0,
        verbose_name="Nombre de succès"
    )
...
class TestCacheModel(models.Model):
    ...
    ttl = models.IntegerField(
        null=True,
        blank=True,
        verbose_name="Durée de vie (secondes)"
    )
...
class TestSecurityModel(models.Model):
    ...
    payload = models.TextField(
        verbose_name="Charge utile du test"
    )
...
class TestUserProfile(models.Model):
    ...
    permissions = models.TextField(
        default='{}',
        verbose_name="Permissions de test"
    )
    created_at = models.DateTimeField(
        auto_now_add=True,
        verbose_name="Date de création"
...
class TestConcurrentModel(models.Model):
    ...
    nom_concurrent = models.CharField(
        max_length=100,
        verbose_name="Nom concurrent"
    )
    valeur_compteur = models.IntegerField(
        default=0,
        verbose_name="Valeur du compteur"
    )
    donnees_partagees = models.TextField(
        default="",
        verbose_name="Données partagées"
    )
    timestamp_creation = models.DateTimeField(
        auto_now_add=True,
        verbose_name="Timestamp de création"
    )
    thread_id = models.CharField(
        max_length=50,
        blank=True,
        verbose_name="ID du thread"
    )
    est_verrouille = models.BooleanField(
        default=False,
        verbose_name="Est verrouillé"
    )
...
class TestSharedResource(models.Model):
    ...
    valeur_partagee = models.IntegerField(
        default=0,
        verbose_name="Valeur partagée"
    )
    nombre_acces = models.IntegerField(
        default=0,
        verbose_name="Nombre d'accès"
    )
...
class TestCompany(models.Model):
    ...
    email_entreprise = models.EmailField(
        verbose_name="E-mail de l'entreprise"
    )
...
class TestEmployee(models.Model):
    ...
    poste_employe = models.CharField(
        max_length=100,
        verbose_name="Poste de l'employé"
    )
    ...
    est_manager = models.BooleanField(
        default=False,
...
class TestSkill(models.Model):
    ...
    description_competence = models.TextField(
        blank=True,
        verbose_name="Description de la compétence"
    )
...
class TestProjectAssignment(models.Model):
    ...
    role_affectation = models.CharField(
        max_length=100,
        verbose_name="Rôle dans l'affectation"
    )
    pourcentage_temps = models.PositiveIntegerField(
        default=100,
        verbose_name="Pourcentage de temps"
    )
...
class FixtureTestReview(BaseTestModel):
    ...
    reviewer_name = models.CharField(
        max_length=100,
        verbose_name="Nom du critique"
    )
    reviewer_email = models.EmailField(
        verbose_name="E-mail du critique"
    )
    ...
    content = models.TextField(
        verbose_name="Contenu de l'avis"
    )
