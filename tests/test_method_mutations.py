...
class TestMethodMutationGeneration(TestCase):
    ...
    def setUp(self):
        ...
        self.tag = Tag.objects.create(name="test-tag", color="#FF0000")
        ...
        self.settings = MutationGeneratorSettings(enable_method_mutations=True)
        self.mutation_generator = MutationGenerator(
            type_generator=self.type_generator, settings=self.settings
        )
    ...
    def test_transaction_handling(self):
        ...
        with patch(
            "rail_django_graphql.generators.mutations.transaction.atomic"
        ) as mock_atomic:
    ...
    def test_type_conversion(self):
        ...
        self.assertEqual(
            self.mutation_generator._convert_python_type_to_graphql(int), graphene.Int
        )
        self.assertEqual(
...
class FixtureTestCategoryMethodMutations(TestCase):
    ...
    def test_deactivate_category_mutation(self):
        ...
        result = mutation_class.mutate(
            None, None, id=self.category.id, reason="Test deactivation"
        )
