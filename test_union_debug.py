...
from django_graphql_auto.generators.inheritance import inheritance_handler
...
def test_union_creation():
    ...
    print(f"LocalClient polymorphic_ctype: {getattr(local_client, 'polymorphic_ctype', 'None')}")
    ...
    # Test union creation
    union_type = inheritance_handler.create_union_for_inheritance_tree(Client, type_generator)
    ...
    if union_type:
        ...
        if hasattr(Client, '_meta'):
            print(f"Has polymorphic_ctype: {hasattr(Client._meta, 'polymorphic_ctype')}")
        ...
        print(f"Query generator field type name: {getattr(client_query_field.type, '__name__', 'No name')}")
        ...
        if hasattr(query_type, '_meta') and hasattr(query_type._meta, 'fields'):
            ...
            if client_field:
                ...
                print(f"Client query field type name: {getattr(client_field.type, '__name__', 'No name')}")
            else:
