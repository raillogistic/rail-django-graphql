                    ...
                    if isinstance(value, list):
                        ...
                        for i, item in enumerate(value):
                            ...
                            if isinstance(item, dict) and 'id' not in item:
                                ...
                                print(f"    Added foreign key: {related_field.field.name} = {instance.pk}")
                                
                                # Handle foreign key fields in the new object data
                                processed_item = {}
                                for key, val in item.items():
                                    print(f"    Processing field: {key} = {val} (type: {type(val)})")
                                    try:
                                        field = related_field.related_model._meta.get_field(key)
                                        print(f"      Field info: {field} (type: {type(field)})")
                                        if isinstance(field, django.db.models.ForeignKey):
                                            ...
                                            if val is None:
                                                ...
                                                print(f"        Converting ID {val} to model instance")
                                                ...
                                                print(f"        Dict value - nested operation")
                                                ...
                                                print(f"        Other type - direct assignment")
                                                processed_item[key] = val
                                        else:
                                ...
                                print(f"    Processed_item types: {[(k, type(v)) for k, v in processed_item.items()]}")
                                
                                print(f"    Calling handle_nested_create with processed_item...")
                                try:
...
def test_debug():
    ...
    # Create test data
    user = User.objects.create_user(username='test_final_user', email='test_final@test.com')
    category = Category.objects.create(name='Test Final Category')
    post = Post.objects.create(title='Test Final Post', content='Test content', category=category)
