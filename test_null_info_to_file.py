...
def test_null_info():
    ...
    with open(output_file, 'w') as f:
        ...
        try:
            ...
            f.write(f"Created LocalClient {local_client.id} without ClientInformation\n")
                    ...
                    for client_data in data['localclients']:
                        ...
                        if client_data['id'] == str(local_client.id):
                            if client_data['info'] is None:
                                f.write("  SUCCESS: Info field correctly returns null!\n")
                            else:
                                f.write(f"  ERROR: Info field should be null but returned: {client_data['info']}\n")
                        f.write("  ---\n")
                else:
