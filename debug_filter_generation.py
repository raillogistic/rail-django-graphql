...
def main():
    ...
    product = Product.objects.create(name="Smartphone Pro", brand=brand, category=category, price=999.99)
    ...
    try:
        ...
        for product in filtered_queryset:
            print(f"    - {product.name} (Brand: {product.brand.name}, Country: {product.brand.country.name})")
