...
def test_polymorphic_field_exclusion():
    ...
    should_include_polymorphic_ctype = type_generator._should_include_field(
        Client, "polymorphic_ctype"
    )
    should_include_client_ptr = type_generator._should_include_field(
        Client, "client_ptr"
    )
    should_include_normal_field = type_generator._should_include_field(Client, "raison")

    print(
        f"   polymorphic_ctype should be excluded: {not should_include_polymorphic_ctype}"
    )
    ...
    try:
        ...
        polymorphic_in_create = any(
            field in create_fields for field in ["polymorphic_ctype", "client_ptr"]
        )
        polymorphic_in_update = any(
            field in update_fields for field in ["polymorphic_ctype", "client_ptr"]
        )
        ...
        if not polymorphic_in_create and not polymorphic_in_update:
            print(
                "   ✅ SUCCESS: Polymorphic fields properly excluded from input types"
            )
        else:
    ...
    try:
        ...
        if create_mutation:
            ...
            if input_arg:
                ...
                polymorphic_in_mutation = any(
                    field in input_fields
                    for field in ["polymorphic_ctype", "client_ptr"]
                )
