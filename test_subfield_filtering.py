...
def test_subfield_filtering_generation():
    ...
    assert isinstance(livres_field, graphene.List), "livres_auteur field should be a List type"
...
def test_subfield_filtering_functionality():
    ...
    try:
        ...
        assert all_reviews.count() == 2, f"Expected 2 reviews, got {all_reviews.count()}"
        ...
        assert filtered_reviews.count() == 1, f"Expected 1 review by Jean Dupont, got {filtered_reviews.count()}"
        
        # Verify the filtered review is the correct one
        filtered_review = filtered_reviews.first()
        assert filtered_review.nom_revieweur == "Jean Dupont", "Filtered review should be by Jean Dupont"
        assert filtered_review.commentaire_avis == "Excellent book!", "Filtered review should have correct content"
...
def test_reverse_relationship_filtering():
    ...
    try:
        ...
        assert filtered_books.count() == 1, f"Expected 1 fiction book, got {filtered_books.count()}"
        ...
        assert filtered_book.titre_livre == "Fiction Book", "Filtered book should have correct title"
