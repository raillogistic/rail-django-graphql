        ...
        if result.errors:
            ...
            for error in result.errors:
                ...
                if hasattr(error, "original_error"):
                    ...
                    print(
                        f"  - Traceback: {traceback.format_exception(type(error.original_error), error.original_error, error.original_error.__traceback__)}"
                    )
        else:
                ...
                for client_data in data["localclients"]:
                    ...
                    if client_data["id"] == str(local_client.id):
                        if client_data["info"] is None:
                            print(
                                "  SUCCESS: Info field correctly returns null for test client!"
                            )
                        else:
                            print(
                                "  ERROR: Info field should be null but returned:",
                                client_data["info"],
                            )
                    print("  ---")
            else:
