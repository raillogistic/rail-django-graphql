...
def test_cascading_polymorphic_resolution():
    ...
    local_client = LocalClient.objects.create(
        raison="Local Client Ltd", 
        test="Local test value"
    )
    print(f"Created LocalClient: {local_client.id} - {local_client.raison} - {local_client.test}")
    ...
    try:
        if query_type and hasattr(query_type, 'fields') and 'clients' in query_type.fields:
