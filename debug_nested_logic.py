                        ...
                        if isinstance(value, list):
                            ...
                            for i, item in enumerate(value):
                                ...
                                if isinstance(item, dict):
                                    if "id" in item:
                                        print(
                                            f"    Updating existing object with id: {item['id']}"
                                        )
                                    else:
                                        ...
                                        print(
                                            f"    Added FK: {related_field.field.name} = {instance.pk}"
                                        )
                                        ...
                                        for key, val in item.items():
                                            print(
                                                f"      {key}: {val} (type: {type(val)})"
                                            )

                                            try:
                                                field = related_field.related_model._meta.get_field(
                                                    key
                                                )
                                                ...
                                                if hasattr(
                                                    ...
                                                    print(
                                                        f"        ForeignKey to {field.related_model}"
                                                    )
                                                    ...
                                                    if val is None:
                                                        ...
                                                        print(
                                                            f"        Converting ID {val} to instance"
                                                        )
                                                        try:
                                                            ...
                                                            print(
                                                                f"        ERROR getting object: {e}"
                                                            )
                                                            processed_item[key] = val
                                                    elif hasattr(val, "pk"):
                                                        processed_item[key] = val
                                                        print(
                                                            f"        -> {val} (already instance)"
                                                        )
                                                    else:
                                                        processed_item[key] = val
                                                        print(
                                        ...
                                        print(
                                            f"    Final processed_item: {processed_item}"
                                        )
                                        print(
                                            f"    Types: {[(k, type(v)) for k, v in processed_item.items()]}"
                                        )
                                        ...
                                        try:
                                            ...
                                            print(
                                                f"    ERROR in handle_nested_create: {e}"
                                            )
                                            raise
...
import rail_django_graphql.generators.mutation_generator as mutation_gen
...
def test_debug():
    ...
    post = Post.objects.create(
        title="Debug Test Post", content="Test content", category=category
    )
