...
def test_polymorphic_type_field():
    ...
    try:
        ...
        if "Client" not in types_found or "LocalClient" not in types_found:
            print(
                f"ERROR: Expected both 'Client' and 'LocalClient' types, got: {types_found}"
            )
            return False
        ...
        if local_client_data:
            print(
                f"Single LocalClient - ID: {local_client_data['id']}, Type: {local_client_data['polymorphicType']}, Raison: {local_client_data['raison']}"
            )

            if local_client_data["polymorphicType"] != "LocalClient":
                print(
                    f"ERROR: Expected 'LocalClient' type, got: {local_client_data['polymorphicType']}"
                )
                return False
        else:
