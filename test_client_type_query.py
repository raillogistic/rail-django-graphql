...
def test_client_type_query():
    ...
    try:
        ...
        client_info = ClientInformation.objects.create(
            client=client, code_postal="12345", pays="France", paysx="FranceX"
        )

        local_client_info = ClientInformation.objects.create(
            client=local_client, code_postal="67890", pays="Germany", paysx="GermanyX"
        )
        ...
        print(
            "✅ SUCCESS: Clients query returns ClientType for both Client and LocalClient instances"
        )
        return True
