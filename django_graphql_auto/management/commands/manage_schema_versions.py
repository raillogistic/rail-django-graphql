...
from django_graphql_auto.core.schema_versioning import schema_version_manager
from django_graphql_auto.core.schema_builder import SchemaBuilder
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        # Créer une version
        create_parser = subparsers.add_parser('create', help='Créer une nouvelle version')
        create_parser.add_argument('version', help='Numéro de version (ex: 1.0.0)')
        create_parser.add_argument('--description', '-d', default='', help='Description de la version')
        create_parser.add_argument('--created-by', default='admin', help='Créateur de la version')
        create_parser.add_argument('--migration-files', nargs='*', default=[], help='Fichiers de migration')
        create_parser.add_argument('--rollback-instructions', help='Instructions de rollback')
        create_parser.add_argument('--activate', action='store_true', help='Activer immédiatement cette version')
        ...
        list_parser.add_argument('--limit', type=int, default=20, help='Nombre maximum de versions à afficher')
        list_parser.add_argument('--format', choices=['table', 'json'], default='table', help='Format de sortie')
        
        # Afficher une version
        show_parser = subparsers.add_parser('show', help='Afficher les détails d\'une version')
        show_parser.add_argument('version', help='Version à afficher')
        show_parser.add_argument('--format', choices=['table', 'json'], default='table', help='Format de sortie')
        ...
        delete_parser.add_argument('--force', action='store_true', help='Forcer la suppression sans confirmation')
        ...
        rollback_parser.add_argument('--force', action='store_true', help='Forcer le rollback sans confirmation')
        ...
        compare_parser.add_argument('--format', choices=['table', 'json'], default='table', help='Format de sortie')
        
        # Historique d'une version
        history_parser = subparsers.add_parser('history', help='Afficher l\'historique d\'une version')
        history_parser.add_argument('version', help='Version à analyser')
        history_parser.add_argument('--format', choices=['table', 'json'], default='table', help='Format de sortie')
        ...
        try:
            ...
            if activate:
                if schema_version_manager.activate_version(version):
                    self.stdout.write(
                        self.style.SUCCESS(f"Version {version} activée")
                    )
                else:
                    self.stdout.write(
                        self.style.ERROR(f"Erreur lors de l'activation de la version {version}")
                    )
    ...
    def handle_list(self, options):
        ...
        if format_type == 'json':
            ...
            # Format tableau
            self.stdout.write("\n" + "="*80)
            self.stdout.write(f"{'Version':<15} {'Active':<8} {'Créé le':<20} {'Créé par':<15} {'Description'}")
    ...
    def handle_show(self, options):
        ...
        if format_type == 'json':
            ...
            self.stdout.write(f"Créé le: {version_obj.created_at.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def handle_delete(self, options):
        ...
        if not force:
            confirm = input(f"Êtes-vous sûr de vouloir supprimer la version {version}? (oui/non): ")
    ...
    def handle_rollback(self, options):
        ...
        if schema_version_manager.rollback_to_version(version):
            self.stdout.write(
                self.style.SUCCESS(f"Rollback vers la version {version} effectué avec succès")
            )
        else:
    ...
    def handle_compare(self, options):
        ...
        if format_type == 'json':
            ...
            self.stdout.write(f"Schéma modifié: {'Oui' if comparison['schema_changed'] else 'Non'}")
            ...
            if comparison['migration_files_v1']:
                self.stdout.write(f"Migrations {version1}: {', '.join(comparison['migration_files_v1'])}")
            
            if comparison['migration_files_v2']:
                self.stdout.write(f"Migrations {version2}: {', '.join(comparison['migration_files_v2'])}")
    ...
    def handle_history(self, options):
        ...
        if format_type == 'json':
            ...
            self.stdout.write(f"Instructions de rollback: {'Disponibles' if history['has_rollback_instructions'] else 'Non disponibles'}")
