...
class GraphQLPerformanceMiddleware(MiddlewareMixin):
    ...
    def __init__(self, get_response=None):
        ...
        self.log_slow_queries = getattr(
            settings, 'GRAPHQL_LOG_SLOW_QUERIES', True
        )
    ...
    def process_response(self, request, response):
        ...
        if self._is_graphql_request(request) and hasattr(request, '_graphql_start_time'):
            try:
    ...
    def _is_graphql_request(self, request) -> bool:
        ...
        return (
            request.path.endswith('/graphql/') or 
            request.path.endswith('/graphql') or
            'graphql' in request.content_type.lower() if hasattr(request, 'content_type') else False
        )
    ...
    def _collect_performance_metrics(self, 
        ...
        if self.enable_memory_tracking and hasattr(request, '_graphql_memory_start'):
            try:
                ...
                memory_usage_mb = (current_memory - request._graphql_memory_start) / 1024 / 1024
                tracemalloc.stop()
            except Exception:
                pass
        ...
        if (self.log_slow_queries and 
            execution_time > self.slow_query_threshold and 
            not error_message):
...
class GraphQLExecutionMiddleware:
    ...
    def _record_field_metrics(self, 
                            info, 
...
def get_performance_middleware_config():
    ...
    MIDDLEWARE = [
        # ... autres middlewares ...
        'django_graphql_auto.middleware.performance_middleware.GraphQLPerformanceMiddleware',
    ]
    ...
    return {
        'middleware_class': 'django_graphql_auto.middleware.performance_middleware.GraphQLPerformanceMiddleware',
        'settings': {
