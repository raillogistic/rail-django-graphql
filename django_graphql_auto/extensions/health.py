...
class HealthChecker:
    ...
    def get_comprehensive_health_report(self) -> Dict[str, Any]:
        ...
        degraded_count = sum(1 for status in all_statuses if status.status == "degraded")
        unhealthy_count = sum(1 for status in all_statuses if status.status == "unhealthy")
        ...
        return {
            ...
            "recommendations": self._generate_recommendations(all_statuses, system_metrics)
        }
    ...
    def _generate_recommendations(self, statuses: List[HealthStatus], metrics: SystemMetrics) -> List[str]:
        ...
        if unhealthy_components:
            recommendations.append(f"Critical: {len(unhealthy_components)} components are unhealthy and require immediate attention")
        
        # Check system resources
        if metrics.cpu_usage_percent > 80:
            recommendations.append("High CPU usage detected. Consider scaling or optimizing queries")
        ...
        if metrics.cache_hit_rate < 70:
            recommendations.append("Low cache hit rate. Review caching strategy and key patterns")
        
        # Check response times
        slow_components = [s for s in statuses if s.response_time_ms > 1000]
        if slow_components:
            recommendations.append(f"{len(slow_components)} components have slow response times (>1s)")
