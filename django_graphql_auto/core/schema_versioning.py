...
class SchemaVersionModel(models.Model):
    ...
    class Meta:
        db_table = 'django_graphql_auto_schema_versions'
        ordering = ['-created_at']
...
class SchemaVersionManager:
    ...
    def create_version(
        ...
        if current_active and current_active.schema_hash == schema_hash:
            logger.warning(f"Le schéma n'a pas changé depuis la version {current_active.version}")
    ...
    def rollback_to_version(self, version: str) -> bool:
        ...
        try:
            ...
            if not schema:
                logger.error(f"Impossible de charger le schéma pour la version {version}")
                return False
