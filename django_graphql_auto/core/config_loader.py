...
def validate_configuration(config: dict) -> dict:
    """
    Valide la configuration DJANGO_GRAPHQL_AUTO avec validation complète du schéma.
    ...
    def validate_section(section_name: str, section_config: dict, section_schema: dict) -> dict:
        validated_section = {}
    ...
    for section_name, section_schema in validation_schema.items():
        if section_name in validated_config:
            ...
            validated_section = validate_section(section_name, section_config, section_schema['schema'])
