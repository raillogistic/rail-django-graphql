...
class RuntimeConfigManager:
    ...
    def _load_initial_config(self) -> None:
        ...
        try:
            # Charger la configuration DJANGO_GRAPHQL_AUTO
            graphql_config = getattr(settings, 'DJANGO_GRAPHQL_AUTO', {})
            ...
            logger.info(f"Configuration initiale chargée: {len(self._config_cache)} paramètres")
    ...
    def set_config(self, key: str, value: Any, user: Optional[str] = None, reason: Optional[str] = None, 
        ...
        try:
            with self._lock:
                ...
                logger.info(f"Configuration mise à jour: {key} = {value} (par {user or 'système'})")
                return True
                
        except Exception as e:
            logger.error(f"Erreur lors de la mise à jour de la configuration {key}: {e}")
            return False
    ...
    def get_change_history(self, key: Optional[str] = None, limit: int = 100) -> List[ConfigurationChange]:
        """
    ...
    def import_config(self, config: Dict[str, Any], user: Optional[str] = None, 
                     reason: str = "Configuration import") -> bool:
        """
        ...
        try:
            ...
            logger.info(f"Configuration importée: {success_count}/{len(config)} paramètres")
    ...
    def validate_config(self, config: Optional[Dict[str, Any]] = None) -> Dict[str, List[str]]:
        ...
        try:
            ...
            for key, value in config_to_validate.items():
                ...
                # Validation spécifique pour DJANGO_GRAPHQL_AUTO
                if key == 'MUTATION_SETTINGS' and isinstance(value, dict):
                    if 'nested_relations_config' in value:
                        ...
                        if not isinstance(nested_config, dict):
                            errors.append("nested_relations_config doit être un dictionnaire")
                        else:
                            for model, enabled in nested_config.items():
                                if not isinstance(enabled, bool):
                                    errors.append(f"nested_relations_config.{model} doit être un booléen")
    ...
    def _notify_config_reload(self, old_config: Dict[str, Any], new_config: Dict[str, Any]) -> None:
    ...
    def _persist_config_change(self, key: str, value: Any) -> None:
        ...
        try:
            config_file = getattr(settings, 'RUNTIME_CONFIG_FILE', 'runtime_config.json')
...
def set_runtime_config(key: str, value: Any, user: Optional[str] = None, 
                      reason: Optional[str] = None, persist: bool = False) -> bool:
    """
